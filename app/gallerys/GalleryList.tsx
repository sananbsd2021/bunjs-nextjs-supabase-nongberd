...
const GalleriesListsPage = () => {
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Are you sure you want to delete this note?")) {
      const { error } = await supabase.from("gallerys").delete().eq("id", id);
  ...
  return (
    <div className="container p-4">
      <h1 className="text-2xl rounded-sm text-white font-bold p-2 mb-4 bg-gradient-to-r from-blue-700 to-white">
        ภาพกิจกรรม
      </h1>
